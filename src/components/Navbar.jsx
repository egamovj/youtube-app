...
export default function Navbar(user) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    console.log("Form submit bo'ldi");
  ...
  return (
    <nav
      className={`fixed top-0 left-0 z-50 w-full py-2 px-4 flex items-center justify-between ${
        bgMode ? "bg-[#17171E]" : "bg-white"
      }`}
    >
      <div className='flex gap-5 items-center h-full mr-6 nvs:mr-0'>
        <i
          onClick={() => {
            setAsideText(!asideText);
            setShowAside(!showAside);
          }}
          className={`fa-solid fa-bars text-[24px] ${
            bgMode
              ? "text-white hover:bg-white/[0.2]"
              : "text-black hover:bg-black/[0.2]"
          }  rounded-full p-3 cursor-pointer md:hidden`}
        ></i>
        <Link to={"/"}>
          <div>
            <svg
      ...
      <form
        className={`w-[830px] flex rounded-[22px] overflow-hidden ${
          bgMode
...
`}
      ...
      >
        <input
          ...
          ref={queryRef}
          placeholder='Search'
          className={`py-[12px] pl-[23px] w-[90%] bg-transparent outline-none ${
            bgMode ? "text-white " : "text-black "
          }`}
        />

        <button type='submit' className='w-[64px]'>
          <i
      ...
      <div className='flex gap-2 items-center h-full ml-5 nvs:ml-0'>
        <button
        ...
        <div
          onClick={() => setShowLogBtn(!showLogBtn)}
          className={`relative flex justify-center items-center w-[40px] h-[40px] rounded-full  ${
            bgMode ? "bg-white/[0.5]" : "bg-black/[0.1]"
          } cursor-pointer`}
        >
          <i
            className={`fa-solid ${
              showLogBtn ? "fa-xmark" : "fa-user"
            } text-[18px]`}
          ></i>
          <div
            className={`absolute z-[99999] bottom-[-10px] translate-y-[100%] backdrop-blur-[5px] text-white ${
              showLogBtn ? "right-0" : "right-[-1000%]"
            } transition-all`}
          >
            {user.user ? (
              <button
                onClick={handleSignOut}
                className={`select-none clip-path rounded-[10px] w-[100px] h-[40px]`}
              >
                Sign out
              </button>
            ) : (
      ...
      <div
        className={`loaderWrapper absolute bottom-0 left-0 w-full h-[5px] bg-transparent ${
          loader ? "block" : "hidden"
        }`}
      >
